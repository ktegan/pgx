...
import wandb
import pgx
from pgx import Strategy, OnePlyStrategy
import optuna
...
class TrainConfig:
    ...
    batch_size: int = 2000             # number of games played in parallel per batch
...
class OptunaConfig:
    num_generations: int = 100
    trials_per_generation: int = 50
    champion_pool_size: int = 6          # we make a pool of recent champions (one per generation) to evaluate candidates
    batch_size: int = 2000
    min_games_per_trial: int = 20000     # minimum number of games to play each optuna trial (split across each champion)
    max_games_per_trial: int = 100000
    initial_rng_key: int = 0
    rel_bounds: float = 0.2
...
def select_active_config(parent_player, config_0, config_1):
    ...
    return type(config_0)(**new_fields)
...
def run_tournament(env_id:str, model_cls:pgx.core.Base, config: TrainConfig, strategy_factory=None, candidate_config_lst=None):
    ...
    while generation < config.num_generations:
        ...
        for field in dataclasses.fields(best_champ):
            val = getattr(best_champ, field.name)
            if hasattr(val, "shape") and len(val.shape) > 0:
                for i, r in enumerate(jnp.ravel(val)):
                    log[f"champion_1/{field.name}_{i}"] = float(r)
            else:
                log[f"champion_1/{field.name}"] = float(val)
...
def dict_to_config(trial_or_params, default_config, mutate_parameters, freeze_parameters, rel_bounds=0.2):
    ...
    return type(default_config)(**config_dict)
...
def optuna_param_search(env_id: str, model_cls: pgx.core.Base, search_config: OptunaConfig, initial_params=None, strategy_factory: Optional[Callable]=None):
    ...
    for generation in range(1, search_config.num_generations + 1):
        ...
        def run_trial(trial):
            ...
            # We use OnePlyStrategy for search
            ...
            while True:
                ...
                #    if (upper_bound < search_config.target_improvement).all():
                #        break
